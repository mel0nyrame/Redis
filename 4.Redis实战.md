...
![1](raw/素材/Redis/images/image.png)
...
![1](raw/素材/Redis/images/image1.png)
...
![1](raw/素材/Redis/images/image2.png)
...
![1](raw/素材/Redis/images/image4.png)
...
![1](raw/素材/Redis/images/image5.png)
...
![1](raw/素材/Redis/images/image6.png)
...
![1](raw/素材/Redis/images/image7.png)
...
使用redis(zset)快速判断用户是否点赞,还能使用zset快速获取点赞前五的用户

### 好友关注业务

#### 关注与取关

被关注和关注者需要有张关系表(普通的crud)

#### 共同关注

可利用Redis的数据结构set,将多个set求交集便是共同关注set
...
- GEO命令
  - GEOADD:添加一个地理信息,包括经度纬度值
  - GEODIST:计算两个位置的距离并返回
  - GEOHASH:将制定值的坐标转换成hash值字符串并返回
  - GEOPOS:将制定值的坐标返回
  - GEOSEARCH:在指定范围内搜索值并按照排序之后返回

>还是使用key:value来使用命令,实际使用多用help

### 根据类型搜索

根据不同类型的商户搜索店铺,可以根据类型当作key,不同坐标为value,将数据塞入redis

## 用户签到业务

### BitMap用法

如果使用mysql一张表来记录用户是否签到,那么占用的内存会非常大

若使用简单的01来统计是否签到,那么内存量就会极大地减少

按照月来统计用户业务状,把每个bit对应每个月的一天,形成映射关系,这种思路成为位图(BitMap)

组成:`key:用户+时间 value:位图`

- BitMap命令
  - SETBIT:向指定位置存入0或1
  - GETBIT:获取指定位置
  - BITCOUNT:统计BitMap中值为1的数量
  - BITFIELD:操作指定位置的值
  - BITFIELD_RO:火速bit数组并以十进制返回
  - BITOP:将多个BitMap进行位运算
  - BITPOST:查找指定位置内第一次出现0或者1的位置

## UV统计业务

### HyperLogLog用法

- UV:Unique Visitor,独立访问人,一个人多次访问一个网站只统计一次
- PV:Page View,页面访问量,统计访问次数

- HyperLogLog:loglog算法派生的统计算法,基于string结构实现但是有误差

- HyperLogLog命令(独一,天然去重)
  - PFADD:添加值到key
  - PFCOUNT:统计key中的值个数
  - PFMERGE:合并key中的值
